export function LogoCarousel() {
    const techStack = [
        {
            name: "GitHub",
            icon: (
                <svg width="40" height="40" viewBox="0 0 24 24" fill="currentColor" className="text-[#cda47b]/80 group-hover:text-[#cda47b] transition-colors">
        ...
        },
        {
            name: "Node.js",
            icon: (
                <div style={{ fontFamily: "'Cinzel', serif" }} className="w-10 h-10 border border-[#cda47b]/40 rounded-lg flex items-center justify-center font-bold text-[#cda47b] bg-[#12100d] shadow-inner">JS</div>
            )
        },
        {
            name: "MongoDB",
    ...
    return (
        <section className="relative w-full max-w-[1400px] mx-auto py-16 px-6 md:px-12 bg-[#000000]">
            <style>{`
            ...
            <div className="mb-10">
                ...
                <p style={{ fontFamily: "'Cormorant Garamond', serif" }} className="text-zinc-400 text-base md:text-lg ml-10">
                    Sacred relics and tools used to forge digital kingdoms.
                </p>
            </div>

            {/* Clean, Borderless Container */}
            <div className="relative w-full py-4 flex items-center justify-between overflow-hidden">
                {/* Masking gradients to fade out edges smoothly into the black background */}
                <div className="relative flex w-full overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_15%,black_85%,transparent)]">
                    ...
                    <div className="flex w-max animate-seamless">
                        {duplicatedStack.map((tech, index) => (
                            <div key={`logo-${index}`} className="flex flex-col items-center justify-center gap-3 group w-28 md:w-40 flex-shrink-0 p-3 rounded-2xl hover:bg-zinc-900/40 transition-all duration-300">
                                <div className="p-3 rounded-xl bg-zinc-900/60 border border-zinc-800/80 group-hover:border-[#cda47b]/40 group-hover:bg-[#12100d] transition-all duration-300 shadow-inner">
                                    {tech.icon}
                                </div>
                                <span style={{ fontFamily: "'Cinzel', serif" }} className="text-xs md:text-sm text-zinc-400 group-hover:text-zinc-200 tracking-wider transition-colors duration-300">
                                    {tech.name}
                                </span>
                            </div>
                        ))}
